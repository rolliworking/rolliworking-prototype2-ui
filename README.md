...
Open http://127.0.0.1:5173 — sign in first (D-181 password + optional webcam / 4-digit PIN switch; device id stored locally). Then Home, Estimates, Hit list, Sales, Intake, Shop time, and Roles. API client attaches stored session token + device id on each call. Configure `PROTOTYPE_AUTH_PASSWORDS` / `PROTOTYPE_AUTH_PINS` on prototype-api. Inventory / PO / QBO live stay UNKNOWN or stub.
