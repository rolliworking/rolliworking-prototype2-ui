...
- **(4) Client-side request close** (2026-06, self-tested via Playwright): `portalCloseRequest` on own `new` requests with reason picker (duplicate → pick sibling request) → `closed_by_client`, audited + replayed; quoted+ shows "Message us about this request" (`/rc/messages?request=`); staff-only `closeRequest` (Client 360 Requests card "Close…"); closed requests folded in RC (`rc-requests-closed-toggle`), dimmed + noted on Client 360; never deleted. Seeds rq-06..rq-09; Pills for new/quoted/closed_by_client.
