...
export const MERGE_FIELDS = ['{{client.first_name}}', '{{watch.brand}}', '{{watch.model}}', '{{estimate.number}}', '{{job.number}}', '{{sub.number}}', '{{so.number}}', '{{pickup.code}}', '{{tracking}}', '{{balance_due}}', '{{shop.name}}'];
...
export const templates: MessageTemplate[] = [
  tpl('intake_confirmation', 'Intake confirmation', 'We’ve received your {{watch.brand}} {{watch.model}} — {{estimate.number}}', 'Hello {{client.first_name}},\n\nYour package arrived safely at {{shop.name}} today (Sub# {{sub.number}}). It now moves to inspection, where we verify the watch against your estimate {{estimate.number}} before any work begins.\n\n— The {{shop.name}} team'),
  tpl('estimate_sent', 'Estimate sent', 'Your estimate {{estimate.number}} — {{watch.brand}} {{watch.model}}', 'Hello {{client.first_name}},\n\nHere is your estimate {{estimate.number}} for your {{watch.brand}} {{watch.model}}. Reply to approve, or let us know if you have questions.\n\n— The {{shop.name}} team'),
  tpl('job_in_progress', 'Job in progress', 'Work has started on your {{watch.model}} ({{job.number}})', 'Hello {{client.first_name}},\n\nA watchmaker has opened the case on your {{watch.brand}} {{watch.model}}. We’ll update you when it reaches final testing.\n\n— The {{shop.name}} team'),
  tpl('back_in_progress', 'Back to in progress (after QC)', 'A short delay on your {{watch.model}} ({{job.number}})', 'Hello {{client.first_name}},\n\nDuring final testing we found something we want to correct before it leaves the shop. It has gone back to the bench and we will update you when testing is complete.\n\n— The {{shop.name}} team'),
  tpl('ready_for_pickup', 'Ready for pickup', 'Your watch is ready for pickup — {{so.number}}', 'Hello {{client.first_name}},\n\nYour {{watch.brand}} {{watch.model}} is ready at the counter. Your pickup code is {{pickup.code}}. Balance due: {{balance_due}}.\n\n— The {{shop.name}} team'),
  tpl('shipped', 'Shipped', 'Your watch has shipped — {{so.number}}', 'Hello {{client.first_name}},\n\nYour watch is on its way, fully insured and signature-required. Tracking: {{tracking}}.\n\n— The {{shop.name}} team'),
];
