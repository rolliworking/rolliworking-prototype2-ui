import { AlertTriangle, ArrowLeft, ArrowRight, Camera, Check, ChevronDown, ClipboardList, Minus, Package, Plus, X } from 'lucide-react';
import { useCallback, useEffect, useState } from 'react';
import { Link } from 'react-router-dom';
import * as api from '@/api/client';
import type { PadCard, PartSuggestion, PartsRequestWithRefs, RoomSummary, SendBackReason } from '@/api/client';
import { Clock, PartDot, PartLocations, PhotoGrid, PinSwitch } from '@/components/rw/RwBits';
...
export default function RwPadPage() {
  const [cards, setCards] = useState<PadCard[]>([]); const [appr, setAppr] = useState<(PartsRequestWithRefs & { onHand: number })[]>([]); const [sum, setSum] = useState<RoomSummary | null>(null); const [tab, setTab] = useState<'board' | 'approvals'>('board'); const [flash, setFlash] = useState<string | null>(null); const [sendBack, setSendBack] = useState<PadCard | null>(null); const [photos, setPhotos] = useState<PadCard | null>(null); const [composer, setComposer] = useState<string | null>(null);
  ...
  return <div data-testid="rw-pad-page" className="flex h-full flex-col text-slate-100">
    ...
    <main className="flex-1 overflow-y-auto p-5">
      {tab === 'board' && <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{cards.map((c) => <article key={c.job.id} data-testid={`pad-card-${c.job.id}`} className="space-y-3 rounded-3xl border border-white/10 bg-[#1f2630] p-5">
        ...
        <div className="grid grid-cols-2 gap-2"><Big testId={`pad-advance-${c.job.id}`} tone="primary" disabled={!c.canAdvance} onClick={async () => { try { await api.padAdvance(c.job.id); say(`${c.job.number} advanced`); } catch (x) { setFlash(x instanceof Error ? x.message : 'Blocked'); } }}>Advance <ArrowRight size={18} /></Big><Big testId={`pad-sendback-${c.job.id}`} disabled={!c.canSendBack} onClick={() => setSendBack(c)}><ArrowLeft size={18} /> Send back</Big>
          <Big testId={`pad-photos-${c.job.id}`} onClick={() => setPhotos(c)}><Camera size={18} /> Photos <span className="font-mono text-sm text-slate-400">{c.photos}</span></Big><Big testId={`pad-request-${c.job.id}`} onClick={() => setComposer(composer === c.job.id ? null : c.job.id)}><Package size={18} /> Request part <ChevronDown size={16} /></Big></div>
