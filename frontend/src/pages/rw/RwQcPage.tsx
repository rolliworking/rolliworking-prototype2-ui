...
export default function RwQcPage() {
  ...
  return <div data-testid="rw-qc-page" className="space-y-3">
    ...
    <Card title="In testing" subtitle={`${b?.qcQueue.length ?? 0} job${b?.qcQueue.length === 1 ? '' : 's'}`} testId="rw-qc-list" bodyClassName="p-0">
      <ul className="divide-y divide-line/70">{b?.qcQueue.map((j) => { const gaps = api.evidenceGaps(j); const req = api.EVIDENCE_REQUIRED[j.kind]; const cr = api.qcRequestGaps(j); return <li key={j.id} data-testid={`rw-qc-${j.id}`} className="flex items-center gap-3 px-4 py-2 text-xs">
        ...
        {cr.length > 0 && <span data-testid={`rw-qc-requests-${j.id}`} className="rounded-sm bg-amber-400 px-1.5 py-0.5 text-[10px] font-bold uppercase text-[#161b22]" title={cr.map((r) => r.text).join(' · ')}>client request unchecked: {cr[0].text}</span>}
        ...
        <Button size="sm" variant="primary" data-testid={`rw-qc-pass-${j.id}`} disabled={gaps.length > 0 || cr.length > 0} title={cr.length ? `QC blocked — client request not checked off: “${cr[0].text}”` : gaps.length ? `Missing: ${gaps.map((g) => api.EVIDENCE_SLOTS.find((s) => s.key === g)!.label).join(', ')}` : undefined} onClick={() => void pass(j)}><CheckCircle2 size={12} /> Pass</Button>
