...
export default function RwBenchPage() {
  ...
  const [settings, setSettings] = useState<BenchSettings>(api.getBenchSettings()); const [tech, setTech] = useState<User | null>(user); const [locked, setLocked] = useState(!user); const [board, setBoard] = useState<BenchBoard | null>(null); const [offline, setOffline] = useState(false); const [cachedAt, setCachedAt] = useState<string | null>(null);
  ...
  const load = useCallback(async () => {
    if (!tech) return;
    try { const b = await api.getBenchBoard(tech.id); setBoard(b); hasBoard.current = true; api.cacheBenchBoard(b); setOffline(false); setCachedAt(null); }
    catch { setOffline(true); if (!hasBoard.current) { const c = api.readCachedBenchBoard(tech.id); if (c) { setBoard(c); hasBoard.current = true; setCachedAt(c.cachedAt); } } }
  }, [tech]);
  useEffect(() => { void load(); }, [load]);
  useEffect(() => { if (!offline) return; const i = window.setInterval(() => void load(), RETRY_MS); return () => window.clearInterval(i); }, [offline, load]);
  useEffect(() => { if (user && !tech) { setTech(user); setLastUser(user); setLocked(false); } }, [user, tech]);
  const relock = useCallback(() => { setThread(null); setGear(false); setLocked(true); }, []);
  useIdle(settings.idleMinutes, !locked && !!tech, relock);
  const unlock = async (u: User) => { if (u.id !== tech?.id) { hasBoard.current = false; setBoard(null); } setLastUser(u); setTech(u); setLocked(false); refreshStation().catch(() => undefined); };
  const openRow = (r: MessageInboxRow) => setThread({ job: r.job, rootId: r.thread.root.id });
  const showLock = locked || !tech;
  return <div data-testid="rw-bench-page" className="bench-kiosk flex h-full flex-col" style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "SF Pro Text", "Helvetica Neue", Inter, sans-serif' }}>
    <header data-testid="bench-header" className="sticky top-0 z-30 border-b border-white/10 bg-[#0b0f14]/95 px-5 py-3 backdrop-blur">
      <div className="flex items-center gap-4">
        ...
        <div><div data-testid="bench-name" className="text-lg font-bold text-slate-100">{settings.benchName}</div><div className="text-xs text-slate-500">Bench Pad · {showLock ? 'locked' : `${tech?.shortName} · ${board ? `${board.inProgress.length} in progress` : 'loading…'}`}</div></div>
    ...
    {offline && <div data-testid="bench-offline-banner" className="flex items-center justify-center gap-2 bg-amber-500/15 px-4 py-1 text-xs font-semibold text-amber-200"><WifiOff size={12} className="bench-pulse" /> reconnecting… <span className="font-normal text-amber-200/70">showing last data{cachedAt && ` · saved ${new Date(cachedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`}</span></div>}
    ...
    : <main data-testid="bench-board" className="flex-1 overflow-y-auto px-5 pb-16 pt-4">
      {board && <div className="mx-auto max-w-5xl space-y-8">
        <BenchSection title="In progress" count={board.inProgress.length} testId="bench-in-progress"><div className="grid gap-3 lg:grid-cols-2">{board.inProgress.map((r) => <JobRowCard key={r.job.id} r={r} section="job" onMessage={(j) => setThread({ job: j })} />)}</div>{!board.inProgress.length && <p className="text-sm text-slate-500">Nothing on your bench right now.</p>}</BenchSection>
        <BenchSection title="Needs attention" count={board.attention.length} testId="bench-attention" tone={board.attention.length ? 'rose' : undefined}><p className="text-xs text-slate-500">stuck = no scan movement in {board.stuckDays} working days · late = past promise date</p><div className="grid gap-3 lg:grid-cols-2">{board.attention.map((r) => <JobRowCard key={r.job.id} r={r} showFlags section="attn" onMessage={(j) => setThread({ job: j })} />)}</div>{!board.attention.length && <p className="text-sm text-slate-500">Nothing stuck or late.</p>}</BenchSection>
      ...
      {!board && <p data-testid="bench-board-empty" className="py-10 text-center text-slate-500">{offline ? 'No saved board on this device yet — reconnecting…' : 'Loading your board…'}</p>}
    </main>}
