...
import type { Assignee, JobWithRefs, Role, User } from '@/api/client';
...
export const NewTaskForm = ({ onCreated, defaultJobId }: { onCreated: () => void; defaultJobId?: string }) => {
  ...
  const [assignee, setAssignee] = useState('user:Vienna');
  const [jobId, setJobId] = useState(defaultJobId ?? '');
  const [dueAt, setDueAt] = useState('');
  const [err, setErr] = useState<string | null>(null);
  useEffect(() => { api.getUsers().then(setUsers); api.getJobs().then((j) => setJobs(j.filter((x) => x.status !== 'closed'))); }, []);
  ...
  return (
    <div data-testid="new-task-form" className="grid grid-cols-[1fr_190px_220px_140px_auto] items-end gap-2">
      ...
      <label className="text-xs text-ink-500">Assign to (user or role)<select data-testid="task-assignee" value={assignee} onChange={(e) => setAssignee(e.target.value)} className={`${field} mt-1 block w-full`}>
        <optgroup label="People">{users.map((u) => <option key={u.id} value={`user:${u.shortName}`}>{u.shortName}</option>)}</optgroup>
        <optgroup label="Roles (resolve to current holders)">{api.ROLES.map((r) => <option key={r} value={`role:${r}`}>{r} → {api.roleHolders(r).map((u) => u.shortName).join(', ')}</option>)}</optgroup>
      </select></label>
