...
export const MessageComposer = ({ onSend, placeholder, testId, autoFocus, onCancel }: { onSend: (text: string, photoUrl?: string) => Promise<void>; placeholder: string; testId: string; autoFocus?: boolean; onCancel?: () => void }) => {
  ...
  return <div data-testid={testId} className="relative space-y-1.5">
    ...
    <div className="flex items-center gap-1.5">
      ...
      <button type="button" data-testid={`${testId}-at`} onClick={() => { onChange(`${text}${text && !text.endsWith(' ') ? ' ' : ''}@`); ta.current?.focus(); }} title="Mention someone" className="inline-flex h-8 items-center gap-1 rounded-sm border border-line px-2 text-xs text-ink-600 hover:bg-canvas"><AtSign size={12} /> Mention</button>
