...
const Head = ({ j, onMessage }: { j: JobWithRefs; onMessage: (j: JobWithRefs) => void }) => (
  <div className="flex flex-wrap items-center gap-2"><span className="font-mono text-xl font-semibold text-slate-100">{j.number}</span><span className="text-slate-300">{j.watch.brand} {j.watch.model}</span><span className="font-mono text-xs text-slate-500">{j.watch.reference}</span><KindPill kind={j.kind} />{j.priority === 'urgent' || j.priority === 'high' ? <span className="rounded-full bg-orange-500/20 px-2 text-xs text-orange-200">{j.priority}</span> : null}
    <button data-testid={`bench-msg-${j.id}`} onClick={() => onMessage(j)} className="ml-auto inline-flex min-h-[40px] items-center gap-1.5 rounded-xl border border-white/15 px-3 text-sm text-slate-200 hover:bg-white/10"><MessageSquare size={14} /> Message</button></div>
);

export const JobRowCard = ({ r, onMessage, showFlags }: { r: BenchJobRow; onMessage: (j: JobWithRefs) => void; showFlags?: boolean }) => (
  <article data-testid={`bench-job-${r.job.id}`} className={`rounded-2xl border p-4 ${r.stuck || r.late ? 'border-rose-500/40 bg-rose-500/[0.06]' : 'border-white/10 bg-white/[0.03]'}`}>
    <Head j={r.job} onMessage={onMessage} />
...
export const SplitCard = ({ s, onMessage }: { s: BenchSplitRow; onMessage: (j: JobWithRefs) => void }) => (
  <article data-testid={`bench-split-${s.job.id}`} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
    <Head j={s.job} onMessage={onMessage} />
...
export const OutsourcedList = ({ rows, onMessage }: { rows: BenchBoard['outsourced']; onMessage: (j: JobWithRefs) => void }) => (
  <ul className="space-y-2">{rows.map((o) => <li key={o.job.id} data-testid={`bench-out-${o.job.id}`} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4"><Head j={o.job} onMessage={onMessage} /><div className="mt-2 flex flex-wrap items-center gap-3 text-sm"><span className="inline-flex items-center gap-1.5 text-slate-200"><Truck size={14} className="text-violet-300" /> {o.vendor}</span><span data-testid={`bench-out-days-${o.job.id}`} className={`rounded-full px-2 py-0.5 font-mono text-xs ${o.daysOut >= 7 ? 'bg-rose-500/25 text-rose-200' : 'bg-white/10 text-slate-300'}`}>{o.daysOut} day{o.daysOut === 1 ? '' : 's'} out</span><span className="text-xs text-slate-500">{o.reason}</span></div></li>)}</ul>
);
